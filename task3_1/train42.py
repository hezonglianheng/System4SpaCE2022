...
def train_epoch(
    ...
    logging.info('epoch: {}, train loss: {}'.format(epoch, losses))
...
def test_epoch(
    ...
    with torch.no_grad():
        for batch in tqdm(test_loader):
            batch_data, batch_labels, batch_qids = batch
            batch_mask = batch_data.gt(0)
            outputs = model(batch_data, batch_mask,
                            labels=batch_labels)
            losses += outputs.loss
    ...
    f1 = metrics.main(true_labels, pred_labels)
    return f1
...
def main(
    ...
    for epoch in range(1, config.epoch_num+1):
        ...
        logging.info(
            'epoch: {}, f1: {}'.format(epoch, epoch_f1)
        )
        f1_improve = epoch_f1 - best_f1
        if f1_improve > 1e-5:
            best_f1 = epoch_f1
            model.save_pretrained(config.step2_model_dir)
        ...
        if (epoch == config.epoch_num) or \
            ...
            logging.info(
                'step2 best f1: {}'.format(best_f1)
            )
            break

    logging.info('train for step2 completed.')